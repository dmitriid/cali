...
public class Neo4J {
    ...
    ConversionManager _cm;

    public Neo4J(final String path) {
    ...
    public Object exec(final OtpErlangObject data) {
        ...
        for(OtpErlangObject current_object : os) {
            if(current_object instanceof OtpErlangTuple) {
                OtpErlangTuple tuple = ((OtpErlangTuple) current_object);
                Object v = addObject(tuple);

                if(null != v){
                    stack.push(v);
                    Object e = updateEdges(stack);
        ...
        if(stack.empty()) return new ArrayList();
        Object o = stack.pop();
    ...
    public Object exec(final ArrayList data){
        ...
        for(Object request : data){
            ...
            if(op.equals("get")) {
                ...
                Object o = setElements(arr.get(1), stack);
                if(null == o) continue;

                if(o instanceof Pair) {
                    Pair p = (Pair) o;
                    if(((String) p.first()).equals("edge")) {
                        do_edge = true;
                        stack.push(p.other());
                    }
                } else {
    ...
    public void execRequest(Object request, Stack stack){
        if(!(request instanceof ArrayList)) return;
        if(((ArrayList)request).size() < 2) return;
    ...
    public Object setElements(Object request, Stack stack){
        if(!(request instanceof ArrayList)) return null;
        if(((ArrayList) request).size() == 0) return null;
        ...
        if(arr.get(0) instanceof ArrayList) {
            ...
            if(op.equals("vertex")){
                ...
                for(Object o : (ArrayList)arr){
                    Vertex v = addVertex((ArrayList)((ArrayList) o).get(1));
                    if(null != v) local_result.add(v);
                }
                result = local_result;
            } else {
                result = new Pair<String, Object>("edge", (ArrayList) arr);    
            }
        } else {
            // {'<-', "label"}
            // {vertex, [{prop, "val"}]} etc.
    ...
    public Object addObject(OtpErlangTuple tuple){
        String op = toString(tuple.elementAt(0));
        if(op.equals("v")) {
            // we are creating a new vertex
            // {v, [{prop1, val1}, {prop2, val2}]}
            Vertex current_vertex = _graphDb.addVertex(null);

            if(tuple.elementAt(1) != null) {
                // we're adding properties
                OtpErlangList l = (OtpErlangList) tuple.elementAt(1);
                setProperties(current_vertex, l);
            }
            return current_vertex;

        } else if(op.equals("->") || op.equals("<-") || op.equals("<->")) {
            // outE (->) from previous node to next
            // or
            // inE (<-) from next node to previous
            // or
            // inE and outE (<->) to both nodes
            // just push it to stack

            Pair p = new Pair(op, toString(tuple.elementAt(1)));
            return p;
        } else if(op.equals("q")) {
            // we are querying the db in Gremlin
            // {q, "$_/inE/outV"}
            String query = toString(tuple.elementAt(1));

            try {
                Object result = _engine.eval(query);
            ...
            // usually to be used with an edge before or after
            OtpErlangObject[] objects = ((OtpErlangList) tuple.elementAt(1)).elements();
            ArrayList result = new ArrayList();
            for(OtpErlangObject o : objects) {
                OtpErlangTuple t = ((OtpErlangTuple) o);
                Object e = addObject(t);
                if(e instanceof ArrayList) {
                    result.addAll((ArrayList) e);
                } else {
                    result.add(e);
                }
            }
    ...
    private Object updateEdges(Stack stack, Object v) {
        if(v instanceof Vertex){
            return updateEdges(stack, (Vertex) v);
        } else if (v instanceof ArrayList) {
            return updateEdges(stack, (ArrayList) v);
        } else {
            return null;
        }
    }

    private Object updateEdges(Stack stack) {
        ...
        if(!(previous_v instanceof Vertex) && !(previous_v instanceof ArrayList))
            return null;
